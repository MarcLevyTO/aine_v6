...
### Stage 5 — Implementation (next)

**Skills:** `bmad-dev-story` (per-story) or `bmad-quick-dev` (more "just build it" flow).

This stage will execute the 7 stories from Stage 4 in order, producing the actual Next.js application code in a subdirectory (most likely `aine/` per the architecture's init command).
...
│   └── implementation-artifacts/      # (will populate during Stage 5)
└── docs/                              # currently empty; reserved for project-level docs
```

The actual Aine app code (when Stage 5 runs) will live in a subdirectory inside this repo, generated by `pnpm create next-app@latest aine`.
...
**Next step:** Run `bmad-dev-story` (or `bmad-quick-dev`) and execute Story 1.1 from `epics.md`.
